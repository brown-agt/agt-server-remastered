...
class ChickenArena(LocalArena):
    ...
    def run(self):
        for p1, p2 in combinations(self.players, 2):
            ChickenArena.run_func_w_time(p1.setup, self.timeout, p1.name)
            ChickenArena.run_func_w_time(p2.setup, self.timeout, p2.name)
            self.run_game(p1, p2)
    ...
    def run_game(self, p1, p2):
        ...
        for _ in range(self.num_rounds):
            p1_action = ChickenArena.run_func_w_time(
                p1.get_action, self.timeout, p1.name, -1)
            p2_action = ChickenArena.run_func_w_time(
            ...
            ChickenArena.run_func_w_time(p1.update, self.timeout, p1.name)
            ChickenArena.run_func_w_time(p2.update, self.timeout, p2.name)
