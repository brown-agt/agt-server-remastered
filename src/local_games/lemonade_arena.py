...
class LemonadeArena(LocalArena):
    ...
    def run(self):
        for p1, p2, p3 in permutations(self.players, 3):
            if self.handin_mode:
                try:
                    LemonadeArena.run_func_w_time(p1.setup, self.timeout, p1.name)
                    LemonadeArena.run_func_w_time(p2.setup, self.timeout, p2.name)
                    LemonadeArena.run_func_w_time(p3.setup, self.timeout, p3.name)
    ...
    def run_game(self, p1, p2, p3):
        for _ in range(self.num_rounds):
            p1_action = LemonadeArena.run_func_w_time(
                p1.get_action, self.timeout, p1.name, -1)
            p2_action = LemonadeArena.run_func_w_time(
                p2.get_action, self.timeout, p2.name, -1)
            p3_action = LemonadeArena.run_func_w_time(
            ...
            LemonadeArena.run_func_w_time(p1.update, self.timeout, p1.name)
            LemonadeArena.run_func_w_time(p2.update, self.timeout, p2.name)
            LemonadeArena.run_func_w_time(p3.update, self.timeout, p3.name)
    ...
    def summarize_results(self):
        ...
        res_summary = [[key, value[0] / (value[1] * self.num_rounds)] if value[1] > 0 else [key, 0]
                       for key, value in total_util_dict.items()]
        sum_df = pd.DataFrame(res_summary)
        sum_df.columns = ['Agent Name', 'Average Utility']
        sum_df = sum_df.sort_values('Average Utility', ascending=False)
        print(f"Results: \n {sum_df}")
